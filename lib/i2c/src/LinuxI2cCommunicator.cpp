...
int LinuxI2cCommunicator::read_block(unsigned char address, unsigned char* buffer)
{
  int result = i2c_smbus_read_i2c_block_data(file_, address, sizeof(buffer), buffer);
  if (result < 0) reportError(errno);
  return result;
}
...
int LinuxI2cCommunicator::write_block(unsigned char address, int count, unsigned char* value)
{
  int result = i2c_smbus_write_i2c_block_data(file_, address, count, value);
  if (result < 0) reportError(errno);
  return result;
}
